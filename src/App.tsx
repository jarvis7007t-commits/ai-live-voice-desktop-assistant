    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: "You are Wardenix, the ultimate AI OS Assistant with FULL NATIVE SYSTEM ACCESS. You operate on a high-performance PC. You have REAL-TIME VISION of the entire screen, including all open tabs, browsers, and applications. You receive commands via VOICE and TEXT CHAT. You MUST NOT guess or simulate actions. You MUST use the 'move_mouse', 'click_mouse', 'type_text', and 'run_command' tools to perform every single step of a task. You can control the PC as if you were the user. Using the terminal ('run_command'), you can open any application or perform advanced system tasks. For coding, you can use 'manage_file' to create/edit code and 'run_command' to execute it. For messaging (like WhatsApp), you can use 'open_app', then navigate using mouse and keyboard tools. You MUST see the result of your action on the screen before proceeding to the next step. You are fast, efficient, and have a bold, helpful personality. You are the master of this PC.",
            ...
            if (message.toolCall) {
              ...
              for (const fc of functionCalls) {
                ...
                if (ipcRenderer) {
                   ...
                   else if (fc.name === 'click_mouse') result = await ipcRenderer.invoke('automation:click', fc.args);
                   else if (fc.name === 'type_text') result = await ipcRenderer.invoke('automation:type', fc.args);
                   else if (fc.name === 'scroll_screen') result = await ipcRenderer.invoke('automation:scroll', fc.args);
                   else if (fc.name === 'open_url') await ipcRenderer.invoke('automation:open_url', fc.args);
                   else if (fc.name === 'system_power') await ipcRenderer.invoke('automation:system_power', fc.args);
                   else if (fc.name === 'open_app') await ipcRenderer.invoke('automation:open_app', fc.args);
                   else if (fc.name === 'press_key') await ipcRenderer.invoke('automation:press_key', fc.args);
                   else if (fc.name === 'manage_file') result = await ipcRenderer.invoke('automation:manage_file', fc.args);
                   else if (fc.name === 'run_command') result = await ipcRenderer.invoke('automation:run_command', fc.args);
                   else if (fc.name === 'set_volume') await ipcRenderer.invoke('automation:set_volume', fc.args);
                   else if (fc.name === 'set_brightness') await ipcRenderer.invoke('automation:set_brightness', fc.args);
                   else if (fc.name === 'toggle_wifi') await ipcRenderer.invoke('automation:toggle_wifi', fc.args);
                   else if (fc.name === 'toggle_bluetooth') await ipcRenderer.invoke('automation:toggle_bluetooth', fc.args);
